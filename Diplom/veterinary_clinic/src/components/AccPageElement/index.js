...
function AccPageElement(props)
{
    ...
    const handleScrollToPersData = (event) => {
        event.preventDefault();
        const aboutUsElement = document.getElementById('persdata');
        if (aboutUsElement) {
            aboutUsElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    };
    const handleScrollToPets = (event) => {
        event.preventDefault();
        const aboutUsElement = document.getElementById('pets');
        if (aboutUsElement) {
            aboutUsElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    };
    ...
    const CreatePatient = async () => {
        try {
          ...
          formData.append('ID_owner', props.data.ID_owner);
    ...
    <div className={stylepageel.accpageelemnt}>
        <h2>
        {props.data.Name?props.data.Name:"Имя"}
        </h2>

        <button onClick={handleScrollToPersData}>
            личные данные
        </button>

        <button onClick={handleScrollToPets}>
            мои питомцы  
        </button>
        <button onClick={handleScrollToRevData}>
            записи на приём
        </button>
        ...
        {isModalCreatePetsVisible && (
                <div className={stylepageel.modal_create_petByUser} style={{ height:'360px' }}>
                    ...
                    <button style={{marginTop:'20px', marginLeft:'285px'}}onClick={CreatePatient}>Создать</button>
                </div>
            )}
            {isModalInfo&&(
