...
import stylepdfa from './PDFA.module.css';

function PdfUploadA(props){
  const analyze = props.analyze || {};
  const medicalCard = analyze.MedicalCard && analyze.MedicalCard[0] ? analyze.MedicalCard[0] : {};
  const medicalCardInfo = medicalCard.MedicalCardInfo || {};
  const analyzes = medicalCard.Analyzes || [];
  const analyzeData = analyzes[0] || {};
  const { Analyze, Date_create } = analyzeData;
    ...
    <div className={stylepdfa.pdf_upload_container}> 
       {Analyze && (
        <div>
            ...
            <p><a style={{color:'black'}}href={process.env.REACT_APP_IMG_URL + Analyze} target="_blank"> Анализ </a></p> 
