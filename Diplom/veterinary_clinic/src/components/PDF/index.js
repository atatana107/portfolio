...
import stylepdf from './PDF.module.css';

function PdfUpload(props){
  const medicalCard = props.pet.MedicalCard && props.pet.MedicalCard.length > 0 ? props.pet.MedicalCard[0] : null;
  const medicalCardInfo = medicalCard ? medicalCard.MedicalCardInfo : null;
  const { Card, Date_create } = medicalCardInfo || {};
  ...
  return (
    <div className={stylepdf.pdf_upload_container}> 
      {Card && (
        <div>
          ...
          <p><a style={{color:'black'}}href={process.env.REACT_APP_IMG_URL + Card} target="_blank"> Медицинская карта</a></p> 
          <p style={{ marginLeft: '200px' }}>создано  {formatDateTime(Date_create)}</p>
          {/* </a> */}
        </div>
       )}
      {!Card && (
      <div className={stylepdf.custom_file_upload_cont} >
        <label style={{width:'330px'}} className={stylepdf.custom_file_upload}>
